...
from meltdown_mitigation import is_criticality_balanced, reactor_efficiency, fail_safe


class MeltdownMitigationTest(unittest.TestCase):
    """Test cases for Meltdown mitigation exeercise."""
        ...
        for variant, data in enumerate(test_data, start=1):
            ...
            with self.subTest(
                ...
                failure_message = (
                    ...
                    f"expected {expected} as the return value."
                )
                self.assertEqual(actual_result, expected, failure_message)

    @pytest.mark.task(taskno=2)
    def test_reactor_efficienty(self):
        voltage = 10
        theoretical_max_power = 10000

        # The numbers are chosen to that current == 10 x percentage
        test_data = (
            (1000, "green"),
            (999, "green"),
            (800, "green"),
