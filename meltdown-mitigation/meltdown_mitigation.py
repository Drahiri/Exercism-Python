...
def reactor_efficiency(
    voltage: int | float, current: int | float, theorecical_max_power: int | float
) -> str:
    """Assess reactor efficiency zone.

    :param voltage: int or float - voltage value.
    :param current: int or float - current value.
    :param theoretical_max_power: int of float - power that corresponds to a 100% efficiency.
    ...
    efficiency = (generated_power / theorecical_max_power) * 100
    if efficiency < 30:
        band = "black"
    elif efficiency < 60:
        band = "red"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    :param neutrons_procuced_per_second: int or float - neutron flux.
